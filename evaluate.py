...
def main():
    ...
    # Final reporting is the only stage that reads the held-out test split.
    (test_dataset,) = build_datasets(cfg, splits=("test",))
    ...
    results = {"loss": loss_total / count, **{k: v / count for k, v in totals.items()}}
    print(
        "test "
        f"IoU={results['iou']:.6f} "
        f"mIoU={results['miou']:.6f} "
        f"HD={results['hd']:.6f} "
        f"ACC={results['acc']:.6f}"
    )
