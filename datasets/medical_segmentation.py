...
def ensure_mmotu_splits(
    ...
    test_path = root / test_split_file
    if train_path.exists() and val_path.exists() and test_path.exists():
        existing = [_read_txt_split(path) for path in (train_path, val_path, test_path)]
        expected_sizes = [train_size, val_size, test_size]
        all_names = [name for split in existing for name in split]
        if [len(split) for split in existing] == expected_sizes and len(all_names) == len(set(all_names)):
            return
    ...
    # Preserve the original 469-image held-out tail and split only the original
    # 1000-image development set into 800 training and 200 validation images.
    development_end = train_size + val_size
    development_pairs = list(pairs[:development_end])
    rng = random.Random(seed)
    rng.shuffle(development_pairs)
    _write_txt_split(train_path, development_pairs[:train_size], root / image_dir)
    _write_txt_split(val_path, development_pairs[train_size:], root / image_dir)
    _write_txt_split(
        test_path,
        pairs[development_end : development_end + test_size],
        root / image_dir,
    )
...
def ensure_isic_split(
    ...
    if split_path.exists():
        ...
        if all(key in existing for key in ("train", "val", "test")):
            return
    pairs = _match_pairs(root / image_dir, root / mask_dir)
    if len(split_ratios) != 3 or any(float(ratio) <= 0 for ratio in split_ratios):
        raise ValueError("dataset.split_ratios must contain positive train/val/test ratios")
    ratio_sum = sum(float(ratio) for ratio in split_ratios)
    ratios = [float(ratio) / ratio_sum for ratio in split_ratios]
    rng = random.Random(seed)
    indices = list(range(len(pairs)))
    rng.shuffle(indices)
    train_count = int(len(indices) * ratios[0])
    val_count = int(len(indices) * ratios[1])
    ...
    payload = {
        ...
        "val": [
            str(pairs[i][0].relative_to(root / image_dir)).replace("\\", "/")
            for i in indices[train_count:val_end]
        ],
        "test": [
            str(pairs[i][0].relative_to(root / image_dir)).replace("\\", "/")
...
def build_datasets(
    ...
    invalid = set(splits) - {"train", "val", "test"}
    if invalid:
        raise ValueError(f"unsupported data splits: {sorted(invalid)}")
    if name == "MMOTU":
        ensure_mmotu_splits(
            root,
            ...
            dataset_cfg["test_split_file"],
            int(dataset_cfg["train_size"]),
            int(dataset_cfg["val_size"]),
            int(dataset_cfg["test_size"]),
            int(exp_cfg.get("seed", 42)),
        )
        split_files = {
            "train": dataset_cfg["train_split_file"],
            "val": dataset_cfg["val_split_file"],
            "test": dataset_cfg["test_split_file"],
        }
    elif name == "ISIC-2018":
        ensure_isic_split(
            root,
            ...
            dataset_cfg["split_ratios"],
            int(exp_cfg.get("seed", 42)),
        )
        split_files = {split: dataset_cfg["split_file"] for split in ("train", "val", "test")}
    else:
